...
class Bi_objective_DFS{
    ...
    */
    void DFS(int current_node, vector<int> path, vector<vector<int>> total_cost_record, vector<int> current_total_cost, vector<vector<int>> *all_paths)
    {
                ...
                if (!isDominated_for_a_star(expanded, expanded_total_cost)){
                    if (!isDominated_for_all_nodes(expanded, total_cost_record, expanded_total_cost, path)) {
                        ...
                        if (expanded == current_d_node) {
                            ...
                            // add to the path
                            // continue searching
                            DFS(expanded, path, total_cost_record, expanded_total_cost, all_paths);
                        }
                    }
                }
            }
    ...
    */
    vector<vector<int>> traverse()
    {
        ...
        DFS(current_s_node, current_path, total_cost_record, start_cost, all_paths);
