...
void DFS(int cur_node, int dest_node, int s2d_c3, int s2d_c4, vector<int> path, vector<int> cost, unordered_map<int, vector<vector<int>>> adjNodes, unordered_map<int, vector<vector<int>>> existingLUBs, vector<vector<int>> *all_paths)
{
    ...
    for (int i = 0; i < cur_node_adj.size(); i++)
    {
        ...
        if (find(path.begin(), path.end(), cur_node_adj[i][0]) == path.end()){
            // update cost
            vector <int> this_cost = {cost[0]+cur_node_adj[i][1],
                                      cost[1]+cur_node_adj[i][2],
                                      cost[2]+cur_node_adj[i][3],
                                      cost[3]+cur_node_adj[i][4]};
...
vector<vector<int>> traverse(int s_node, int d_node, int s2d_ub1, int s2d_ub2, unordered_map<int, vector<vector<int>>> adjNodes, unordered_map<int, vector<vector<int>>> Existing_LUBs)
{
    ...
    DFS(s_node, d_node, s2d_ub1, s2d_ub2, cur_path, cur_cost, adjNodes, Existing_LUBs, all_paths);
...
int main() {
    ...
    for (int i=0; i<100; i++) {
        ...
        cout << "s_node:" << s_node_id << endl;
        cout << "d_node:" << d_node_id << endl;
    ...
    /*
    for (int i=0; i<all_pair_path.size(); i++){
        for (int j=0; j<all_pair_path[i].size(); j++) {
            cout << (all_pair_path)[i][j] << " ";
        }
        cout << endl;
    }*/
